...
import { TestAddExcelFileHubDatas } from "../../services/HubServices"; 
// ─────────────────────────────
//  MAIN COMPONENT
// ─────────────────────────────
export default function ExcelEntryTest() {
  ...
  const validateExcel = (data) => {
    ...
    if (!requiredHeaders.every((h) => headers.includes(h))) {
      errors.push(
        " Excel must have columns: GC_NO | INWARD | GC_DATE | PARTY_NAME | CR_NO | PACKAGE | WEIGHT | LR_AMOUNT | REBATE | AFTER_REBATE | OTHERS | CR_AMOUNT | FROM_ADDRESS | PAID_AMOUNT | BALANCE_AMOUNT"
      );
      return errors;
    }
  ...
  /*const mapToDto = (row) => ({
    ...
    cashReceiptNo: row.CR_NO,
    pks: row.PACKAGE,
    weight: row.WEIGHT,
    lorryReceiptAmount: row.LR_AMOUNT,
    rebate: row.REBATE,
    afterRebate: row.AFTER_REBATE,
    others: row.OTHERS,
    cashReceiptAmount: row.CR_AMOUNT,
  ...
  const mapToDto = (row) => ({
    lorryReceiptNo: Number(row.GC_NO),

    inwardNo: String(row.INWARD || "").trim(),

    lorryReceiptDate: row.GC_DATE, // already ISO from parseJoinDate()

    partyName: row.PARTY_NAME || "",

    cashReceiptNo: Number(row.CR_NO || 0),
    pks: Number(row.PACKAGE || 0),
    weight: Number(row.WEIGHT || 0),

    lorryReceiptAmount: Number(row.LR_AMOUNT || 0),

    // FIX: convert float → integer
    rebate: Math.round(Number(row.REBATE || 0)),

    afterRebate: Number(row.AFTER_REBATE || 0),
    others: Number(row.OTHERS || 0),

    cashReceiptAmount: Number(row.CR_AMOUNT || 0),
  ...
  const handleSubmit = async () => {
    ...
    try {
        // lr_number	inward	lr_date	party_name	cr_number	pks	weight	lr_amount	
  // rebate	after_rebate	others	cr_amount	
    // from_addres	payment_amount	balance_amount
      const response = await TestAddExcelFileHubDatas(dtoData);
      const { data, status } = response;

      if (status === 200) {
        setInsertedExcelResultCount(data.insertedCountTest);
        setInsertedExcelResultList(data.insertedXlDatasTest);
        setSkippedExcelResultCount(data.skippedCountTest);
        setSkippedExcelResultList(data.skippedXlDatasTest);
        setIsXlResult(true);
  ...
  return (
    <div className="pt-1">
      ...
      {errors.length === 0 && excelData.length > 0 && (
        <ExcelVerificationTableTest excelData={excelData} handleSubmit={handleSubmit} />
      )}
        ...
        <div className="flex flex-col gap-8 mt-6">
          <div>
            ...
            {insertedExcelResultCount > 0 && (
              <ExcelResultTableTest excelResult={insertedExcelResultList} bgcolor="bg-green-400" />
            )}
          </div>
          ...
          <div>
            ...
            {skippedExcelResultCount > 0 && (
              <ExcelResultTableTest excelResult={skippedExcelResultList} bgcolor="bg-red-400" />
            )}
          </div>
        </div>
      )}
...
//  PREVIEW TABLE
// ─────────────────────────────
export const ExcelVerificationTableTest = ({ excelData, handleSubmit }) => {
  const Th =
...
//  RESULT TABLE
// ─────────────────────────────
export const ExcelResultTableTest = ({ excelResult, bgcolor }) => {
            ...
            >
              <thead>
                <tr>
                  ...
                  <th className={Th}>CR NO</th>
                  <th className={Th}>Package</th>
                  <th className={Th}>Weight</th>
                  <th className={Th}>LR Amount</th>
                  <th className={Th}>Rebate</th>
                  <th className={Th}>After Rebate</th>
                  <th className={Th}>Others</th>
                  <th className={Th}>CR Amount</th>
                  <th className={Th}>From Address</th>
                  <th className={Th}>Paid Amount</th>
                  <th className={Th}>Balance Amount</th>
                </tr>
              </thead>
              <tbody className="dark:text-white">
